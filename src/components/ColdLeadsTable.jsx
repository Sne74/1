...
import { coldLeadsData } from '../data/mockData';
import './Table.css';

const ColdLeadsTable = () => {
  const [showDropdown, setShowDropdown] = useState(false);
  const [selectedFilter, setSelectedFilter] = useState('Driver');

  const filterOptions = ['Driver', 'Dealer', "NBFC's", "OEM's", 'Driver(OEM)', 'Dealer(OEM)', "NBFC's(OEM)", 'CRM'];

  return (
    <div className="table-container">
      <div className="table-header">
        <h3 className="table-title">Cold Leads (6)</h3>
          ...
          <tbody>
            {coldLeadsData.map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  <div className="referred-text">{row.referred}</div>
                  <div className="ref-id-text">{row.refId}</div>
                </td>
              </tr>
            ))}
          </tbody>
